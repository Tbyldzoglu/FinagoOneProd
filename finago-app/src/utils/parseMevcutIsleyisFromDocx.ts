/**
 * Mevcut İşleyiş parser - DOCX dosyasından metin içeriği çıkarır
 */

import mammoth from 'mammoth';

export interface MevcutIsleyisResult {
  content: string;
  validation: {
    found: boolean;
    mode: 'strict' | 'scan';
...
/**
 * Mevcut İşleyiş başlığını DOCX'te arar
 */
function findMevcutIsleyisHeader(dom: Document): Element | null {
  const searchTerms = [
    'mevcut isleyis',
    'mevcut işleyiş',
    'current process',
  ...
  console.log('🔍 MEVCUT İŞLEYİŞ: Başlık aranıyor...', searchTerms);
  ...
  for (let i = 0; i < headerElements.length; i++) {
    ...
    for (const term of searchTerms) {
      if (text.includes(term)) {
        console.log(`🎯 MEVCUT İŞLEYİŞ BAŞLIK BULUNDU: "${originalText}" (term: ${term})`);
        return element;
      }
    }
  }
  ...
  for (let i = 0; i < allElements.length; i++) {
    ...
    for (const term of searchTerms) {
      if (text.includes(term)) {
        console.log(`🎯 MEVCUT İŞLEYİŞ ELEMENT BULUNDU: "${originalText}" (term: ${term})`);
        return element;
      }
    }
  }

  console.log('❌ Hiçbir yerde Mevcut İşleyiş başlığı bulunamadı');
  return null;
}
...
function extractContentAfterHeader(header: Element, dom: Document): string {
  console.log('📝 MEVCUT İŞLEYİŞ: Başlık altındaki içerik toplaniyor...');
  ...
  console.log(`✅ MEVCUT İŞLEYİŞ SONUÇ: ${content.length} paragraf, ${result.length} karakter`);
 ...
 */
function scanForMevcutIsleyisContent(dom: Document): string {
  console.log('🔍 MEVCUT İŞLEYİŞ SCAN: İçerik aranıyor...');
  
  const keywords = [
    'mevcut', 'isleyis', 'işleyiş', 'current', 'existing', 'process',
    'süreç', 'akış', 'adım', 'prosedür', 'procedure', 'workflow'
  ];
  ...
  for (let i = 0; i < allElements.length; i++) {
    ...
    for (const keyword of keywords) {
      if (normalizedText.includes(keyword)) {
        score += keyword === 'mevcut' || keyword === 'isleyis' || keyword === 'işleyiş' ? 5 : 2;
      }
    }
    ...
    // Mevcut/İşleyiş kombinasyonu bonus
    if ((normalizedText.includes('mevcut') && normalizedText.includes('isleyis')) ||
        (normalizedText.includes('mevcut') && normalizedText.includes('işleyiş'))) {
      score += 10;
    }
 ...
 */
export async function parseMevcutIsleyisFromDocx(file: File): Promise<MevcutIsleyisResult> {
  console.log(`🔍 DOCX Mevcut İşleyiş Parse Başlıyor: ${file.name}`);
  ...
  try {
    ...
    const header = findMevcutIsleyisHeader(dom);
    
    if (header) {
      validation.mode = 'strict';
      validation.found = true;
      validation.matchedLabels.push('mevcut_isleyis_basligi');
      ...
      content = scanForMevcutIsleyisContent(dom);
    ...
    // Sonuçları logla
    console.log('📊 Mevcut İşleyiş Parse Sonucu:', {
