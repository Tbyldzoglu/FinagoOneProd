...
interface PlanlananIsleyisParseResult {
  found: boolean;
  mode: 'strict' | 'scan';
  content: string;
...
// PLANLANAN İŞLEYİŞ başlığını bul
function findPlanlananIsleyisHeader(doc: Document): Element | null {
  console.log('🔍 PLANLANAN İŞLEYİŞ: Başlık aranıyor...');
  ...
  console.log('🔍 PLANLANAN İŞLEYİŞ: Başlık aranıyor...', searchTerms);
  ...
  for (let i = 0; i < headings.length; i++) {
    ...
    for (const term of searchTerms) {
      if (normalized.includes(term)) {
        console.log(`🎯 PLANLANAN İŞLEYİŞ BAŞLIK BULUNDU: "${text}" (term: ${term})`);
        return heading;
      }
    }
  }
  ...
  for (let i = 0; i < allElements.length; i++) {
    ...
    for (const term of searchTerms) {
      if (normalized.includes(term) && text.length < 100) { // Kısa başlık benzeri metinler
        console.log(`🎯 PLANLANAN İŞLEYİŞ ELEMENT BULUNDU: "${text}" (term: ${term})`);
        return element;
      }
    }
  }
  
  console.log('❌ PLANLANAN İŞLEYİŞ başlığı bulunamadı');
  return null;
}

// Başlık sonrası içeriği topla
function extractContentAfterHeader(doc: Document, headerElement: Element): string {
  console.log('📝 PLANLANAN İŞLEYİŞ: Başlık altındaki içerik toplaniyor...');
  ...
  console.log(`✅ PLANLANAN İŞLEYİŞ SONUÇ: ${content.length} paragraf, ${result.length} karakter`);
  return result;
}

// SCAN mode: Dokümanı tara ve skorla
function scanForPlanlananIsleyisContent(doc: Document): string {
  console.log('🔍 SCAN Mode: Planlanan İşleyiş içeriği aranıyor...');
  
  const keywords = ['planlanan', 'plan', 'gelecek', 'yeni', 'future', 'planned', 'new'];
...
export async function parsePlanlananIsleyisFromDocx(file: File): Promise<PlanlananIsleyisParseResult> {
  console.log('🔍 DOCX Planlanan İşleyiş Parse Başlıyor:', file.name);
  ...
  try {
    ...
    const headerElement = findPlanlananIsleyisHeader(doc);
    
    if (headerElement) {
      const content = extractContentAfterHeader(doc, headerElement);
      
      if (content && content.trim().length > 0) {
        console.log('📊 Planlanan İşleyiş Parse Sonucu:', {
          found: true,
          mode: 'strict',
          contentLength: content.length,
          matchedLabels: ['Planlanan İşleyiş'],
          errors: [],
          warnings: []
        });
    ...
    const scanContent = scanForPlanlananIsleyisContent(doc);
    
    if (scanContent && scanContent.trim().length > 0) {
      console.log('📊 Planlanan İşleyiş Parse Sonucu (SCAN):', {
        found: true,
        mode: 'scan',
    ...
    return {
      ...
      contentLength: 0,
      matchedLabels: [],
      errors: ['Planlanan İşleyiş içeriği bulunamadı'],
      warnings: []
    };
